          ...
          <nav className="my-6">
            <ul className="flex justify-between font-bold">
              {data?.topics?.map((topic: Topic, index: number) => (
                <li key={index} className="hover:text-secondary"><Link href="/">{topic.topic_name}</Link></li>
              ))}
            </ul>
          </nav>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
              {data.comments && data.comments.length > 0 ?
                data.comments.map((comment: Comment, index: number) => (
                    <CommentCardComponent key={index}
                                          ...
                                          article_author_avatar_img_src={comment.articles?.members?.avatar_url}
