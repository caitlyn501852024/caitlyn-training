...
export default function MyPostsComponent({
  ...
  return (
    <>
            ...
            {selectedTopics.map((topic, index) => (
              <div key={index} className="badge badge-primary gap-1">
                ...
                <button className="hover:cursor-pointer"
                        onClick={() => onTopicsChangeAction(selectedTopics.filter(t => t !== topic))}>
                  ✕
                </button>
              </div>
            ))}
      ...
      <p
        className="text-gray-500 text-sm mb-4">共 {articles?.pagination?.totalCount ?? 0} 筆結果・第 {articles?.pagination?.currentPage ?? 1} 頁
        /
        共 {articles?.pagination?.totalPages ?? 1} 頁・目前顯示第 {articles?.pagination?.startItem ?? 0} - {articles?.pagination?.endItem ?? 0} 筆結果</p>
      <section className="mb-4">
        {articles?.articleData.length > 0 ? articles?.articleData.map((article: Article, index: number) => (
